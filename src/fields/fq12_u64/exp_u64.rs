...
const fn constants(num_io: usize) -> ExpU64StarkConstants {
    let start_flags_col = 108 * N_LIMBS;
    let num_main_cols = start_flags_col + NUM_FLAGS_U64_COLS;
...
#[derive(Clone, Copy)]
pub struct Fq12ExpU64Stark<F: RichField + Extendable<D>, const D: usize, const NUM_IO: usize> {
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, const NUM_IO: usize>
    Fq12ExpU64Stark<F, D, NUM_IO>
{
    pub fn new() -> Self {
        Self {
...
impl<F: RichField + Extendable<D>, const D: usize, const NUM_IO: usize> Stark<F, D>
    for Fq12ExpU64Stark<F, D, NUM_IO>
where
    [(); num_columns(NUM_IO)]:,
    [(); num_public_inputs(NUM_IO)]:,
{
    ...
    fn eval_ext_circuit(
        ...
        cur_col = 0;
        for i in (0..2 * c.num_io).step_by(2) {
            let fq12_exp_io = read_fq12_exp_u64_io(vars.get_public_inputs(), &mut cur_col);
            let is_ith_input = lv[get_pulse_col(c.start_io_pulses_col, i)];
            let is_ith_output = lv[get_pulse_col(c.start_io_pulses_col, i + 1)];
            (0..12).for_each(|i| {
                vec_equal_circuit(
